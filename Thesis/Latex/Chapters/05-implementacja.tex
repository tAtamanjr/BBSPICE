...
Implementacja aplikacji została podzielona zgodnie z podziałem odpowiedzialności opisanym w \cref{chap:projekt-programu}. Kod źródłowy obejmuje moduły związane z interfejsem użytkownika, parserem, stemplami elementów, macierzami oraz solverem. Taki układ odpowiada naturalnemu przepływowi danych: od tekstowego opisu obwodu, przez reprezentację elementów, aż do układu równań rozwiązywanego numerycznie i prezentacji wyniku. Język Swift został wykorzystany ze względu na typowanie statyczne, obsługę błędów i dobre wsparcie dla aplikacji macOS \cite{swiftlanguage}.
...
Podczas zamiany tokenów na wartości liczbowe parser obsługuje również przedrostki mnożące. Dzięki temu użytkownik może zapisać pojemność jako \texttt{100n}, rezystancję jako \texttt{1k}, a czas symulacji jako \texttt{4m}. Przedrostki są rozpoznawane z uwzględnieniem wielkości liter, więc \texttt{M} i \texttt{m} oznaczają różne mnożniki. Niepoprawny przedrostek jest traktowany tak samo jak inny błąd typu parametru i prowadzi do błędu parsera z numerem linii.
...
Wprowadzenie kontekstu upraszcza rozwój programu, ponieważ element nie musi posiadać wielu wersji metod dla różnych analiz. Źródło \texttt{ACVS} odczytuje z kontekstu aktualny czas i na tej podstawie oblicza chwilową wartość sinusoidy. Kondensator wykorzystuje krok czasowy oraz poprzednie napięcie, aby utworzyć model zastępczy dla analizy \texttt{.tran}. Tranzystor BJT korzysta z bieżącego przybliżenia rozwiązania, ponieważ jego zlinearyzowany stempel zależy od aktualnych napięć bazy, kolektora i emitera.
...
W kolejnych krokach czasowych solver tworzy nowy kontekst zawierający aktualny czas, krok czasowy i rozwiązanie z poprzedniego kroku. Następnie składa macierz globalną z tych samych stempli, ale ich wartości mogą być inne niż w poprzednim kroku. Dla źródła sinusoidalnego zmienia się wartość wymuszenia, dla kondensatora zmienia się wkład zależny od historii, a dla elementów nieliniowych zmienia się lokalna linearyzacja.

W przypadku elementów nieliniowych, takich jak tranzystor bipolarny, solver używa iteracji Newtona-Raphsona. W każdej iteracji stemple są obliczane dla aktualnego przybliżenia rozwiązania, a następnie rozwiązywany jest zlinearyzowany układ równań. Proces kończy się po osiągnięciu zadanej tolerancji albo po przekroczeniu maksymalnej liczby iteracji. W aktualnej implementacji limit ustawiono na 150 iteracji, co pozwala stabilnie policzyć układ wtórnika emiterowego wykorzystany jako przykład analizy czasowej.

\section{Interfejs użytkownika i wizualizacja wyników}
\label{sec:implementacja-wizualizacja}

Interfejs użytkownika został zaimplementowany w SwiftUI. Główny widok ma trzy podstawowe stany: edycję opisu układu, prezentację wyniku punktu pracy oraz prezentację wyniku analizy czasowej. W stanie edycji użytkownik może wpisać netlistę ręcznie albo przeciągnąć plik \texttt{.txt}; po upuszczeniu pliku jego zawartość zastępuje tekst w edytorze. Do rozpoznawania przeciąganego typu pliku wykorzystano mechanizm Uniform Type Identifiers dostępny w systemowych bibliotekach Apple \cite{swiftui,uniformtypeidentifiers}.
